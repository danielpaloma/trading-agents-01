...
def onBarUpdate(bars, hasNewBar):
    ...
    if bars[-1].date > last_bar:
        ...
        ####################### Trading Strategy ###########################
        # position signal, represented by:
        # df["position"] is [-1,0,1]
...
def execute_trade(target):
    ...
    if target > 0: # GOING LONG
        if current_pos == 0: # from NEUTRAL
            go_long_short(side = "BUY", target = target, sl_price = sl_price)
        elif current_pos < 0: # from SHORT:
            cancel_orders() # cancel sl orders
            go_neutral(side = "BUY", trades = current_pos)
            go_long_short(side = "BUY", target = target, sl_price = sl_price)
    elif target < 0: # GOING SHORT
        if current_pos == 0: # from NEUTRAL
            go_long_short(side = "SELL", target = abs(target), sl_price = sl_price)
...
def go_long_short(side, target, sl_price): # NEW Go Long/Short starting from Neutral posistion
...
def cancel_orders(): # cancel SL orders
    try:
        sl_cancel = ib.cancelOrder(stopLoss)
    except:
        pass

def BracketOrder(parentOrderId, childOrderId1,
        ...
        #Case 2: SL event
            ...
            if exp_pos != current_pos:
                ...
                try:
                    ...
                    pass
                print("Session Stopped (SL Event).")
                ib.disconnect()
                break
            else:
                pass
        #Case 3: no streaming update in the last 120 seconds (networking issue??)
